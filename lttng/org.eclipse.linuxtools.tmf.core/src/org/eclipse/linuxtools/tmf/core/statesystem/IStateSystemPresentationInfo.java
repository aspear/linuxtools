...
public interface IStateSystemPresentationInfo {
     ...
     * 0 item is the root, and increasing items are deeper nodes in the tree.  The values are
     * static for this instance.  The values of the states and such are all of the same
    ...
    /** If the event contains a "context switch", then switch to the current context
     * tracked by the given value returned.  If the return value is null, then there is no
     * context switch for this event.  Otherwise, the string array returned is always the same
     * dimensions as the context hierarchy, but may contain null values for portions.  If this
     * is the case, then it is assumed that the portions that are not null in the returned
     * value replace the portions of the current active context.  Each complete context value
     * contains a stack of contexts as well, and this event switches to a different stack, like
     * an operating system context switch.
     *
     * Note though that this does not mean that the all
    ...
    /**
     * If this routine returns non null, then the String array returned, which is the same
     * dimentions as the context hierarchy, contains a non null string for the context value(s)
     * that should be pushed on the stack as well as the new values
     * @param eventTypeName
     * @param event
     * @return
     */
    ...
    /**
     * If this routine returns non null, then the String array returned, which is the same
     * dimensions as the context hierarchy, contains a non null string value for
     * @param eventTypeName
     * @param event
     * @return
     */
    ...
    /**
     * If the given event contains a state change for the currently active context, then
     * this routine returns info for the new state, otherwise it returns null.
     * @param eventTypeName the name of the type of the event.  This is passed in as an
     * efficiently convenience.
