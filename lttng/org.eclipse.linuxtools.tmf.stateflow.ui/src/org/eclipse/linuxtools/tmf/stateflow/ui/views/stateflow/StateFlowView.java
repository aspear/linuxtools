...
public class StateFlowView extends TmfView {
	...
	private static ITmfStateSystem getDDStateSystem(ITmfTrace trace) {
		return trace.getStateSystems().get(DataDrivenStateInput.STATE_SYSTEM_ID);
	}
	...
	public void synchToTime(final TmfTimeSynchSignal signal) {
		...
		for (ITmfTrace trace : traces) {
			...
			ITmfStateSystem ss = getDDStateSystem(trace);
			if (ss != null) {
	...
	private IStateSystemPresentationInfo getPresentationInfoForTrace(ITmfTrace trace) {
		ITmfStateSystem ssq = getDDStateSystem(trace);
		if (ssq == null) {
			return null;
		}
	...
	private void buildStatusEvents(ITmfTrace trace, StateFlowEntry entry, IProgressMonitor monitor) {
		ITmfStateSystem ssq = getDDStateSystem(entry.getTrace());
